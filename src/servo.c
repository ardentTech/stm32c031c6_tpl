...
static uint16_t angle_to_pulse(const Servo *servo, uint16_t angle);

static void Servo_update_position(Servo *servo, uint16_t position_us);
...
Servo Servo_init(
    ...
    Servo servo = (Servo){timer, channel, min_position_us, max_position_us, 0, 0};
    servo.step_us = angle_to_pulse(&servo, STEP_DEGREES);
    return servo;
}

void Servo_rotate(Servo *servo, const uint16_t angle) {
...
// TODO unit test
static uint16_t angle_to_pulse(const Servo *servo, const uint16_t angle) {
    return angle * (servo->max_position_us - servo->min_position_us) / 180;
}

// TODO unit test
static void Servo_update_position(Servo *servo, const uint16_t position_us) {
